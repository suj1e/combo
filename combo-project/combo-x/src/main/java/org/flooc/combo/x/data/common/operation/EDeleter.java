...
import org.flooc.combo.x.validate.DeleteGroup;
 ...
 */
@Slf4j
public class EDeleter<E, I> extends AbstractEOperation implements Applyer<E>, Loader<E>, Handler<E>,
		Executor<E> {
	...
	private Consumer<E> successHook = t -> {
		if (log.isDebugEnabled()) {
			log.debug("Successfully delete dataObject of {}", t.getClass().getSimpleName());
		}
	};

	private Consumer<? super Throwable> errorHook = e -> {
		log.error("Failed to delete dataObject of {}, errorMsg: {}", t.getClass().getSimpleName(), e.getMessage());
	};

	public EDeleter(CmdRepo<E, I> cmdRepo) {
