...
import static org.flooc.combo.x.data.common.qry.filler.DataFillProcessType.CONCURRENT;
import static org.flooc.combo.x.data.common.qry.filler.DataFillProcessType.ORDERLY;
...
import org.flooc.combo.x.data.common.qry.filler.DataFillConfig;
...
import org.flooc.combo.x.data.common.qry.filler.process.group.processor.ConcurrentDataFillGroupProcessor;
import org.flooc.combo.x.data.common.qry.filler.process.group.processor.DataFillGroupProcessor;
import org.flooc.combo.x.data.common.qry.filler.process.group.processor.OrderlyDataFillGroupProcessor;
...
public class DefaultDataFillGroupProcessorFactory implements DataFillGroupProcessorFactory {
	...
	private DataFillGroupProcessor buildDataFillGroupProcessor(Class<?> config, DataFillConfig dataFillConfig,
			List<DataFillElementProcessor> processors) {
		// 有序
		if (dataFillConfig == null || dataFillConfig.processType() == ORDERLY) {
			if (log.isDebugEnabled()) {
				log.debug("DataFill user orderly processor for {}", config);
			}
			return new OrderlyDataFillGroupProcessor(config, processors);
		}
		Assert.state(dataFillConfig.processType() == CONCURRENT, "DataFill config illegal for " + config);
		// 并发
		if (log.isDebugEnabled()) {
